...
for i, n in enumerate(ns):
    ...
    for j, nu in enumerate(nus):
        ...
        ctr = 0
        u = models.calc_ana_sol_4(x, 0.0)[:, 0]
        v_0 = models.calc_ana_sol_t_init_4(x)[:, 0]
        ...
        for k, t_now in enumerate(ts):
            ...
            u_ana = models.calc_ana_sol_4(x, t_now)[:, 0]
...
for i, t in enumerate(ts):
    ...
    if i in [0]:
        pyplot.plot(*calc_log_line(3.0, 8192.0, 0.5e-2, -2.0), linewidth=0.5, color="black", label="Slope $-2$")
    else:
        pyplot.plot(*calc_log_line(3.0, 8192.0, 1.2e-2, -2.0), linewidth=0.5, color="black", label="Slope $-2$")
    pyplot.semilogx()
    pyplot.semilogy()
    pyplot.xlabel("$n$")
    pyplot.ylabel("Error")
