...
beta = models.get_beta_2()
cond = models.get_cond_2()
...
for n in ns:
    ...
    u_ana_all = models.driver_ana2_all_2(n)
...
pyplot.plot(*calc_log_line(ns[0], ns[-1], 0.7e-1, -1.0), linewidth=0.5, color="black", label="Slope $-1$")
pyplot.plot(*calc_log_line(ns[0], ns[-1], 3.0e-3, -2.0), linewidth=0.5, color="black", linestyle="--", label="Slope $-2$")
pyplot.semilogx()
pyplot.semilogy()
pyplot.xlabel("$x$")
pyplot.ylabel("Error")
